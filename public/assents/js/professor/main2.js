...
// Cria o worker
const worker = new Worker('./public/assents/js/professor/worker/opencv_marker_worker.js');
...
function processarRespostas(respostas) {
    ...
    respostas.forEach((prova, i) => {
        ...
        const tr_header = `
            <tr class="tr-header"><th colspan="6">DISCIPLINA</th></tr>
            <tr class="tr-header">
                <th>Questão</th>
            ...
            Object.keys(options).forEach((key) => {
                if (key !== '') { // Ignora a opção de 'Nenhuma opção selecionada'
                    ...
                    <td>
                        ...
                        <input type="radio" name="${inputName}" id="${inputName}" value="${value}" ${isChecked}>
                    </td>
                `;
                }
            });
